...
class DocumentStore:
    ...
    def __init__(self, db_path: Optional[str] = None):
        ...
        if DocumentStore._shared_embedding_function is None:
            print("--- DocumentStore.__init__: Initializing embedding function ---")
            DocumentStore._shared_embedding_function = create_embedding_function()
            print(f"✅ Shared embedding function initialized ({EMBEDDING_MODEL})")
        
        self.embedding_function = DocumentStore._shared_embedding_function

        # Initialize shared client (connection pooling). ChromaDB is preferred, but it
        # can be hard to install on free Python 3.12 hosts, so keep a functional JSON fallback.
        if DocumentStore._shared_client is None:
            if chromadb is None or Settings is None:
                print("⚠️  ChromaDB not installed. Using lightweight JSON document store fallback.")
                DocumentStore._shared_client = _JsonDocumentClient(self.db_path, self.embedding_function)
            else:
                ...
                try:
                    ...
                    print("⚠️  Falling back to lightweight JSON document store.")
                    DocumentStore._shared_client = _JsonDocumentClient(self.db_path, self.embedding_function)
...
class _JsonDocumentClient:
    """Small Chroma-compatible fallback for free deployments without ChromaDB."""
    ...
    def get_or_create_collection(self, name: str, embedding_function=None, metadata=None):
        return self.get_collection(name, embedding_function=embedding_function, metadata=metadata)

    def get_collection(self, name: str, embedding_function=None, metadata=None):
        if name not in self._collections:
            self._collections[name] = _JsonDocumentCollection(
                self.db_path / f"{name}.json",
                embedding_function or self.embedding_function,
                metadata or {}
            )
...
class _JsonDocumentCollection:
    """Persist documents and do basic cosine search with the configured embedding API."""

    def __init__(self, file_path: Path, embedding_function, metadata: Dict[str, Any]):
    ...
    def _save(self):
        self.file_path.write_text(json.dumps(self._records, ensure_ascii=False), encoding="utf-8")
    ...
    def query(self, query_texts: List[str], n_results: int, where: Optional[Dict[str, Any]] = None):
        ...
        all_documents, all_metadatas, all_distances, all_ids = [], [], [], []
        candidates = [record for record in self._records if self._matches_where(record.get("metadata", {}), where)]

        for query_embedding in query_embeddings:
            scored = []
            for record in candidates:
                similarity = self._cosine_similarity(query_embedding, record.get("embedding", []))
                scored.append((1.0 - similarity, record))
            scored.sort(key=lambda item: item[0])
            top_records = scored[:n_results]
            all_documents.append([record["document"] for _, record in top_records])
            all_metadatas.append([record.get("metadata", {}) for _, record in top_records])
            all_distances.append([distance for distance, _ in top_records])
            all_ids.append([record["id"] for _, record in top_records])
    ...
    def get(self, where: Optional[Dict[str, Any]] = None, limit: Optional[int] = None):
        records = [record for record in self._records if self._matches_where(record.get("metadata", {}), where)]
        if limit:
    ...
    def delete(self, ids: List[str]):
        delete_ids = set(ids)
        self._records = [record for record in self._records if record["id"] not in delete_ids]
        self._save()
