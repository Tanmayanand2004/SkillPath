...
from src.utils.llm_client import get_llm_client, get_llm_model, get_llm_provider
...
def _call_openai(prompt: str, timeout: int = 45) -> str:
    """Fallback to the configured OpenAI-compatible provider."""
    if get_llm_provider() == "groq":
        raise RuntimeError("Groq does not provide live web search; using static snapshot")
