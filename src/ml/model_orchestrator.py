...
class ModelOrchestrator:
    ...
    def __init__(self, api_key: Optional[str] = None, provider: Optional[str] = None):
        ...
        Args:
            ...
            provider: Optional provider name ('groq', 'openai', or 'deepseek')
        """
        ...
        if self.provider in ('groq', 'openai'):
            ...
            if not self.api_key:
                raise ValueError("API key is required. Set GROQ_API_KEY for free Groq inference.")
            
            print("--- ModelOrchestrator.__init__: Preparing to initialize ChatOpenAI ---")
            print(f"--- ModelOrchestrator.__init__: Provider: {self.provider}, API Key: SET, Model: {DEFAULT_MODEL}, Temp: {TEMPERATURE}, Max Tokens: {MAX_TOKENS} ---")
            # self.llm = ChatOpenAI(
            ...
            raise ValueError(f"Unsupported provider: {self.provider}. Use 'groq', 'openai', or 'deepseek'.")
    ...
    def init_language_model(self, model_name: Optional[str] = None, temperature: Optional[float] = None):
        ...
        try:
            if self.provider in ('groq', 'openai'):
                ...
                if self.provider == 'groq' and OPENAI_BASE_URL:
    ...
    def switch_provider(self, provider: str, api_key: Optional[str] = None, model_name: Optional[str] = None):
        ...
        try:
            ...
            # OpenAI is the primary provider now
                ...
                raise ValueError(f"Unsupported provider: {provider}. Use 'groq', 'openai', or 'deepseek'.")
            ...
            if self.provider != 'groq' and GROQ_API_KEY:
                print("Falling back to Groq provider")
                return self.switch_provider('groq', GROQ_API_KEY, model_name or DEFAULT_MODEL)
    ...
    def generate_response(
        ...
        try:
            ...
            print("DEBUG: About to make OpenAI-compatible API call using direct implementation...")
    ...
    def generate_structured_response(
        ...
        try:
            ...
            if self.provider in ('groq', 'openai'):
                api_key = self.api_key
                if api_key:
                    print(f"DEBUG: Using {self.provider} API key: SET")
                else:
                    print(f"DEBUG: WARNING - No {self.provider} API key found!")
                    
            elif self.provider == 'deepseek':
                api_key = DEEPSEEK_API_KEY
                if api_key:
                    print("DEBUG: Using DeepSeek API key: SET")
                else:
                    print("DEBUG: WARNING - No DeepSeek API key found!")
                    
            if self.provider in ('groq', 'openai'):
                from src.direct_openai import generate_completion
                print("Attempting to generate OpenAI-compatible completion...")
                response_text = generate_completion(
                    prompt=full_prompt,
                    ...
                    model=self.model_name,
                    temperature=temp,
                    max_tokens=max_tokens or (4000 if is_learning_path else MAX_TOKENS),
