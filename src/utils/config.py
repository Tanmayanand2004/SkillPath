...
# Groq exposes an OpenAI-compatible chat-completions API. When a Groq key is
# present, keep legacy OpenAI-client call sites working by setting the same
# environment variables the OpenAI SDK/LangChain already understand.
if GROQ_API_KEY and not os.getenv("OPENAI_API_KEY"):
    os.environ["OPENAI_API_KEY"] = GROQ_API_KEY
if GROQ_API_KEY and not OPENAI_BASE_URL:
...
# Avoid failing at import time so the UI, database migrations, and health check
# can still run. API calls will validate keys when a model is actually used.
if not DEV_MODE and not OPENAI_API_KEY:
    print("WARNING: Set GROQ_API_KEY for free AI inference, or OPENAI_API_KEY for paid OpenAI.")

# Default model provider (can be 'groq', 'openai', or 'deepseek')
DEFAULT_PROVIDER = os.getenv("DEFAULT_PROVIDER", "groq" if GROQ_API_KEY else "openai").lower()

# Model configuration
DEFAULT_MODEL = os.getenv(
    "DEFAULT_MODEL",
    "openai/gpt-oss-20b" if DEFAULT_PROVIDER == "groq" else "gpt-4o-mini",
)
...
REASONING_MODEL = os.getenv("REASONING_MODEL", DEFAULT_MODEL)  # For complex reasoning
SIMPLE_MODEL = os.getenv("SIMPLE_MODEL", DEFAULT_MODEL)  # For simple tasks
...
QUERY_REWRITE_MODEL = os.getenv("QUERY_REWRITE_MODEL", DEFAULT_MODEL)
...
COMPRESSION_MODEL = os.getenv("COMPRESSION_MODEL", DEFAULT_MODEL)
