...
try:
    ...
    from src.utils.llm_client import get_llm_provider
except Exception:
...
class SemanticCache:
    ...
    def __init__(
        ...
        # Initialize OpenAI for embeddings only when explicitly using OpenAI.
        # Groq is chat-only, and the free deployment should not require paid
        # embedding calls. Without embeddings, semantic caching is disabled.
        if OPENAI_AVAILABLE and get_llm_provider() == "openai":
