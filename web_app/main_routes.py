...
from src.data.skills_database import (
    SKILLS_DATABASE,
    get_all_categories,
    ...
    if not hasattr(current_app, "path_generator"):
        ...
        try:
            ...
            current_app.logger.error(
                f"Failed to initialize LearningPathGenerator in main_routes: {e}"
            )
...
def get_resource_manager():
    if not hasattr(current_app, "resource_manager"):
        ...
        try:
            ...
            current_app.logger.error(
                f"Failed to initialize ResourceManager in main_routes: {e}"
            )
...
def _index_template_context(**extra):
    categories = get_all_categories()
    skills_by_category = {
        category: get_skills_by_category(category) for category in categories
    }
    context = {
...
def _normalize_time_commitment(value):
    if not value:
        return value

    normalized = value.strip().lower()
    if normalized in TIME_COMMITMENTS:
        return normalized

    for key, label in TIME_COMMITMENTS.items():
        label_normalized = label.strip().lower()
        if normalized == label_normalized or normalized == label_normalized.replace(
            " per ", "/"
        ):
            return key

    common_labels = {
        "1-2 hours/week": "minimal",
        "1-2 hours per week": "minimal",
        "3-5 hours/week": "moderate",
        "3-5 hours per week": "moderate",
        "6-10 hours/week": "substantial",
        "6-10 hours per week": "substantial",
        "10+ hours/week": "intensive",
        "10+ hours per week": "intensive",
    }
    return common_labels.get(normalized, value)


def _get_anonymous_paths_dir():
    """Ensure the anonymous paths directory exists and return it."""
    paths_dir = os.path.join(current_app.root_path, "static", "paths")
    os.makedirs(paths_dir, exist_ok=True)
    return paths_dir


def _store_anonymous_path(path_id, path_data):
    """Store an anonymous learning path to the filesystem."""
    try:
        paths_dir = _get_anonymous_paths_dir()
        file_path = os.path.join(paths_dir, f"{path_id}.json")
        with open(file_path, "w") as f:
            json.dump(path_data, f)
        current_app.logger.info(f"Stored anonymous path to filesystem: {path_id}")
    except Exception as e:
        current_app.logger.error(f"Failed to store anonymous path {path_id}: {e}")


def _get_anonymous_path(path_id):
    """Retrieve an anonymous learning path from the filesystem."""
    try:
        paths_dir = _get_anonymous_paths_dir()
        file_path = os.path.join(paths_dir, f"{path_id}.json")
        ...
        current_app.logger.error(f"Failed to retrieve anonymous path {path_id}: {e}")
    return None


@bp.route("/")
def index():
    return render_template("index.html", **_index_template_context())
...
def generate_path():
    ...
    if (
        os.getenv("ENABLE_MOCK_DATA") == "True"
        and request.form.get("topic") == "mock_path"
    ):
    ...
    if not path_generator:
        ...
        return (
            jsonify({"success": False, "error": "LearningPathGenerator not available"}),
            500,
        )
    ...
    try:
        ...
        time_commitment = _normalize_time_commitment(data.get("time_commitment"))
        duration_weeks = data.get("duration_weeks")  # Get user-specified duration
        ai_provider = os.getenv("DEFAULT_PROVIDER", "groq")
        ...
        path_data = validated_path.model_dump()
        ...
        if getattr(current_user, "is_authenticated", False):
            ...
            existing_path = UserLearningPath.query.filter_by(
                user_id=getattr(current_user, "id", None), id=path_id
            ).first()
        ...
        if "current_path" in session:
            del session["current_path"]
        return render_template(
            "index.html", **_index_template_context(error=error_message)
        )
    except Exception as e:
...
def generate_task():
    ...
    try:
        ...
        if getattr(current_app, "redis", None) is None:
            current_app.logger.error(
                "REDIS_URL not configured on web service; background queue unavailable"
            )
            return (
                jsonify(
                    {
                        "error": "Background queue unavailable",
                        "detail": "REDIS_URL not configured on web service",
                    }
                ),
                503,
            )
...
def generate_stream():
    ...
    user_id = getattr(current_user, "id", None) if is_authenticated else None
    ...
    def generate():
        try:
            ...
            if is_authenticated and user_id:
                try:
                    ...
                    if existing_path:
                        ...
                        db.session.commit()
                        current_app.logger.info(
                            f"Created {len(milestones)} progress entries"
                        )
                except Exception as db_error:
                    current_app.logger.error(
                        f"Database error in SSE stream: {str(db_error)}"
                    )
                    db.session.rollback()
            ...
            current_app.logger.info(
                f"SSE: Sending completion with redirect URL: {result_url}"
            )
            current_app.logger.info(
                f'SSE: Session has current_path: {"current_path" in session}'
            )
            ...
            error_msg = (
                f"AI response validation failed. Please try again or refine your topic."
            )
...
def save_learning_path():
    ...
    if getattr(current_user, "is_authenticated", False) and path_id:
    ...
    if not path_data and path_id:
        path_data = _get_anonymous_path(path_id)
    ...
    if getattr(current_user, "is_authenticated", False):
        ...
        if existing_path:
            # Update existing path
            existing_path.path_data_json = (
                path_data  # Use path_data_json field name from the model
            )
            existing_path.title = path_data.get("title", "Untitled Path")
            existing_path.topic = path_data.get("topic", "General")
            existing_path.last_accessed_at = (
                datetime.utcnow()
            )  # Use last_accessed_at instead of updated_at
            db.session.commit()
...
def load_learning_path(path_id):
    ...
    file_path = os.path.join(
        current_app.root_path, "static", "paths", f"{path_id}.json"
    )
...
def load_paths():
    if request.method == "GET":
        if getattr(current_user, "is_authenticated", False):
            user_paths = UserLearningPath.query.filter_by(user_id=getattr(current_user, "id", None)).all()
...
def dashboard():
    ...
    user_paths = (
        UserLearningPath.query.filter_by(user_id=getattr(current_user, "id", None), is_archived=False)
    ...
    # Get archived paths
    archived_paths = (
        UserLearningPath.query.filter_by(user_id=getattr(current_user, "id", None), is_archived=True)
        .order_by(UserLearningPath.created_at.desc())
        .all()
    )
    ...
    for path in user_paths:
        ...
        next_action = "Review Path"
        if path.path_data_json and "milestones" in path.path_data_json:
            for idx, m in enumerate(path.path_data_json["milestones"]):
                status_entry = next(
                    (e for e in progress_entries if e.milestone_identifier == str(idx)),
                    None,
                )
                if not status_entry or status_entry.status != "completed":
                    next_action = f"Continue: {m.get('title', 'Next Milestone')}"
                    break
        ...
        paths_with_progress.append(
            {
                ...
                "topic": path.topic,
                "expertise_level": (
                    path.path_data_json.get("expertise_level", "Beginner")
                    if path.path_data_json
                    else "Beginner"
                ),
                "created_at": (
                    path.created_at.strftime("%Y-%m-%d")
                    if path.created_at
...
def upload_document():
    ...
    if not resource_manager:
        return (
            jsonify({"success": False, "error": "ResourceManager not available"}),
            500,
        )

    if "document" not in request.files:
        return (
            jsonify({"success": False, "error": "No document part in the request"}),
            400,
        )
    ...
    if file:
        ...
        try:
            ...
            current_app.logger.error(
                f"Error processing uploaded document {filename}: {str(e)}"
            )
            return (
                jsonify(
    ...
    return (
        jsonify(
            {"success": False, "error": "File upload failed for an unknown reason."}
        ),
        500,
    )
...
def result():
    ...
    if requested_id and getattr(current_user, "is_authenticated", False):
        user_path = UserLearningPath.query.filter_by(
            user_id=getattr(current_user, "id", None), id=requested_id
        ).first()
        if user_path:
            path_data = user_path.path_data_json
            current_app.logger.info(f"Loaded path from database: {requested_id}")

    # Try anonymous storage for requested_id
    if not path_data and requested_id:
        path_data = _get_anonymous_path(requested_id)
        if path_data:
            current_app.logger.info(
                f"Loaded anonymous path from filesystem: {requested_id}"
            )

    # Fall back to session path_id if not found in database or anonymous storage
    if not path_data:
        ...
        if session_path_id:
            ...
            if getattr(current_user, "is_authenticated", False):
                user_path = UserLearningPath.query.filter_by(
                    user_id=getattr(current_user, "id", None), id=session_path_id
                ).first()
                if user_path:
                    path_data = user_path.path_data_json
                    current_app.logger.info(
                        f"Loaded path from database using session ID: {session_path_id}"
                    )

            # Try anonymous storage for session_path_id
            if not path_data:
                path_data = _get_anonymous_path(session_path_id)
                if path_data:
                    current_app.logger.info(
                        f"Loaded anonymous path from filesystem using session ID: {session_path_id}"
                    )
    ...
    if getattr(current_user, "is_authenticated", False) and path_id:
        ...
        try:
            ...
            milestone_progress_entries = MilestoneProgress.query.filter_by(
                user_id=getattr(current_user, "id", None), learning_path_id=path_id
            ).all()
            ...
            for entry in milestone_progress_entries:
                ...
                if entry.completed:
                    ...
                    if (
                        milestone_key not in progress_map
                        or progress_map[milestone_key] != "completed"
                    ):
            ...
            current_app.logger.warning(
                f"Could not fetch MilestoneProgress (table may not exist): {e}"
            )

    progress_percentage = (
        int((completed_count / total_milestones) * 100) if total_milestones else 0
    )
...
def api_save_path():
    ...
    return jsonify(
        {"success": True, "message": "Path saved successfully (placeholder)."}
    )
...
def update_progress():
    ...
    if (
        not data
        or "path_id" not in data
        or "milestone_identifier" not in data
    ...
    user_path = UserLearningPath.query.filter_by(
        id=path_id, user_id=getattr(current_user, "id", None)
    ).first()
    ...
    if (
        milestone_index is None
        or milestone_index < 0
        or milestone_index >= len(milestones)
    ):
        return (
            jsonify({"status": "error", "message": "Invalid milestone identifier"}),
            400,
        )

    # Find or create the progress entry
    progress = LearningProgress.query.filter_by(
        user_learning_path_id=path_id, milestone_identifier=milestone_identifier
    ).first()
...
def archive_path():
    ...
    # Find the path
    path = UserLearningPath.query.filter_by(user_id=getattr(current_user, "id", None), id=path_id).first()
...
def chatbot_query():
    ...
    if current_app.config.get("DEV_MODE"):
        ...
        learning_path += (
            f"- Why {request.json.get('topic', 'Untitled Topic')} is important\n\n"
        )
    ...
    try:
        ...
        # Initialize OpenAI-compatible client (Groq by default)
...
4. Remind them they can generate a new path if major changes are needed"""
        ...
        return jsonify(
            {"reply": response_text, "intent": "general_query", "confidence": 0.9}
        )
        ...
        return (
            jsonify(
                {
                    "reply": "Sorry, the AI provider is temporarily unreachable. Please try again in a moment.",
        ...
        if path_id:
            ...
            if path_data:
                ...
                if user_path:
                    ...
                    completed_count = sum(
                        1 for p in progress_records if p.status == "completed"
                    )
                    ...
                    for i, milestone in enumerate(milestones):
                        progress = next(
                            (
                                ...
                                if p.milestone_identifier == str(i)
                            ),
                            None,
                        )
                        if not progress or progress.status != "completed":
                            current_milestone = milestone.get(
...
"""
                for i, milestone in enumerate(milestones[:5], 1):  # Show first 5
                    ...
                    path_context += (
                        f"{status} {i}. {milestone.get('title', f'Milestone {i}')}\n"
                    )
...
Provide a helpful, context-aware response that acknowledges our conversation history."""
        ...
        print(
            f"💾 Saved conversation messages (tokens: {tokens_used}, time: {response_time_ms}ms)"
        )
...
Provide a clear, helpful answer."""
            ...
            for chunk in orchestrator.generate_response_stream(
                prompt=prompt, relevant_documents=context_texts, temperature=0.7
            ):
...
def direct_chat():
    ...
    Supports multi-turn conversations via conversation_history and persistent DB storage.
    """
    try:
        data = request.get_json()
        ...
        current_app.logger.info(
            f"Chat request - Mode: {mode}, Message: {user_message[:100]}"
        )
        ...
        # Build conversation context from client-side history
        ...
        # Also try DB history for authenticated users
        if not history_messages and getattr(current_user, "is_authenticated", False):
            try:
                db_history = ChatMessage.get_conversation_history(
        ...
        if mode == "Path":
            system_prompt = "You are SkillPath AI, a Learning Path Specialist. Help users create and modify personalized learning paths. When you have enough info, add: GENERATE_PATH: [topic] | [expertise_level] | [time_commitment] | [learning_style]. Be concise and use markdown."
        else:
            system_prompt = "You are SkillPath AI, a friendly learning assistant. Help with learning questions, study tips, and career development. When asked to generate a learning path, gather details then add: GENERATE_PATH: [topic] | [expertise_level] | [time_commitment] | [learning_style]. Be warm, concise, and use markdown."
        ...
        completion = client.chat.completions.create(
            model=get_llm_model(), messages=messages, temperature=0.7, max_tokens=2500
        )
        ...
        if "GENERATE_PATH:" in ai_reply:
            ...
            if len(parts) >= 4:
                ...
                parameters = {
                    "topic": parts[0].strip(),
                    "expertise_level": parts[1].strip().lower(),
                    "time_commitment": parts[2].strip(),
                    "learning_style": parts[3].strip().lower(),
                }
...
def view_path(path_id):
    ...
    session["current_path"] = path.path_data_json
    return redirect(url_for("main.result"))


@bp.route("/job_market", methods=["GET"])
def job_market():
    """Return real-time job-market snapshot."""
...
# Progress routes have been moved to web_app/controllers/progress_controller.py
...
"""
        ...
        safe_title = "".join(
            c
            for c in path_data.get("title", "learning-path")
